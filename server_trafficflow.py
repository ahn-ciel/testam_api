...
def startup_event():
    ...
    log_file = os.path.join(
        log_dir, f"inferTrafficflow_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    )
...
# 개발 할때 편의 위해
@app.get("/predict-trafficflow1", response_class=ORJSONResponse)
def predict_trafficflow_base_config():
    # 매번 로드
    args = load_args_from_json("/TESTAM/config_trafficflow.json")  
    dataloader, scaler = load_data(args)
    result = prediction(args, app.state.engine, dataloader, scaler, app.state.device)
    return jsonable_encoder(result["run_time"], custom_encoder={np.ndarray: lambda x: x.tolist()})

# 운영 할때 편의 위해
@app.get("/predict-trafficflow2", response_class=ORJSONResponse)
def predict_trafficflow_base_parameters(data_path: str, scaler: List[float] = Query(...)):
    # 1. 기존 상태 가져오기
