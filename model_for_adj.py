...
class gcn(nn.Module):
    ...
    def forward(self,x,support):
        ...
        return h
# class gcn(nn.Module):
#     def __init__(self, c_in, c_out, dropout, supports=None, supports_len=3, order=2):
#         super(gcn, self).__init__()
#         self.nconv = nconv()
#         self.order = order
#         self.dropout = dropout
#         self.supports = supports  # ✅ supports를 멤버 변수로 저장

#         c_in = (order * supports_len + 1) * c_in
#         self.mlp = nn.Linear(c_in, c_out)

#     def forward(self, x, supports=None):  # ✅ supports를 forward에서 받을 수도 있도록 설정
#         if supports is None:
#             supports = self.supports  # ✅ supports가 None이면 self.supports 사용

#         if supports is None or len(supports) == 0:
#             raise ValueError("supports (adjacency matrix) cannot be None or empty")
...
class STModel(nn.Module):
    ...
    """
    def __init__(self, hidden_size, supports_len, num_nodes, dropout, layers, out_dim = 1, in_dim = 2, spatial = True, activation = nn.ReLU()):
...
class TESTAM(nn.Module):
    ...
    TESTAM model
    """
    def __init__(self, num_nodes, dropout=0.3, in_dim=2, out_dim = 1, hidden_size = 32, layers = 3, prob_mul = False, max_time_index = 288, supports = None, **args):
        ...
        self.supports_len = len(self.supports) +2
    ...
    def forward(self, input, gate_out = False):
        ...
        new_supports = self.supports + [g1, g2]
