...
def evaluate_dimension(yhat_np, realy_np, axis, label, check=None):
    ...
    for i in range(dim):
        ...
        metrics = util.metric(torch.tensor(pred), torch.tensor(real))
...
def main(args):
    ...
    log_file = args.save+ f'_train_{today}_{args.num_nodes}.log'
    ...
    if args.adjdata and os.path.exists(args.adjdata):
        sensor_ids, _, adj_mx = util.load_adj(args.adjdata, args.adjtype)
        args.num_nodes = len(sensor_ids)
        supports = [torch.tensor(sup).to(device) for sup in adj_mx]    
            
    dataloader = util.load_dataset(args.data_dir, args.batch_size, args.batch_size, args.batch_size) 
    ...
    torch.save(engine.model.state_dict(), args.save+f"_exp{args.expid}_best_{round(his_loss[bestid],2)}.pth")
    api_response={
...
if __name__ == "__main__":
    ...
    logging.info(f"Total time spent: {t2 - t1:.4f} seconds")
